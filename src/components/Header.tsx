...
const Header: React.FC<HeaderProps> = ({ cartItemsCount, onCartClick, onMenuClick }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-hava-yellow backdrop-blur-md shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <button
            onClick={onMenuClick}
            className="flex items-center space-x-2 text-ramen-dark hover:text-white transition-colors duration-200"
          >
            {loading ? (
